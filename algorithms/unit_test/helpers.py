...
class Util:
    ...
    def interpolate_2d_lookup(d_in: pd.DataFrame, x_in, y_in):
        ...
        x_axis = [float(re.sub('[^0-9.]*','',re.sub('(?<=[0-9])_(?=[0-9])','.',v))) for v in d_in.columns.tolist()[1:]]
        ...
        return \
            x_low_y_low_val+ \
            (x_up_y_low_val-x_low_y_low_val)* \
            section_x + \
    ...
    def type_converter(v: ..., k: str=""):
        if type(v) == np.int64 or type(v) == int:
            if 'premium' in k or \
                'modifier' in k or \
                ('tria' in k and 'class' not in k) or \
                'weight' in k or \
                'bound' in k or \
                v==0:
                return float(v)
...
class UnitTest:
    ...
    def get_full_compare_paths_for_list_and_attributes(ave_test_policy, comparison_base_list_path, child_attributes_list):
