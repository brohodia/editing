...
def data(scen_inputs, nes_inputs):
    in_input = scen_inputs.insuredInterest
    nested_input = nes_inputs.lossHistory
...
def agg_list(
    ...
    for key in keys:
        setattr(
            ...
            key,
            sum(
                [getattr(el.ratingResult,key) 
                for el in list_obj
                if getattr(el.ratingResult,key,None) is not None
            ])
        )
